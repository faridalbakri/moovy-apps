...
export default function Login() {
  ...
  useEffect(() => {
    if (location.state?.message) {
      triggerAlert(location.state.message);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error(error);
      triggerAlert(error?.response?.data?.message || "Invalid email or password");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div style={{ width: '100%', height: '100vh', position: 'relative', overflow: 'hidden', backgroundColor: 'black' }}>
      {alert && (
        <AnimatedContent
          ...
          threshold={0.2}
          delay={0.3}
          className="p-4.5 rounded-full w-fit absolute top-5 right-5 bg-black/10 shadow-lg shadow-black/20"
        >
      ...
      <div className="absolute top-55 left-1/2 transform -translate-x-1/2 w-100 text-white  backdrop-blur-xl  shadow-lg shadow-black/30 border border-black/20 rounded-xl z-50">
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 p-6 bg-black/20 rounded-xl shadow-lg shadow-black/30">
